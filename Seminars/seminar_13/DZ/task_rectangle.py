from class_error import ValError
...
class Rectangle:
    ...
    def __str__(self):
        return f"В результате - прямоугольник со сторонами: {self._side_a}," \
               f" {self._side_b}; периметром: {self.get_perimeter()}; площадью: {self.get_area()}"
